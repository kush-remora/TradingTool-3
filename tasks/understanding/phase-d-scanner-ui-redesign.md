...
The next consistency fix is schema vocabulary. `phase_c_watchlist` originally used several misleading legacy names like `marketcapname`, `high_252d`, `min_20d_high`, and `brackets2`. The stack should use the cleaned names end-to-end so the CSV, database, Kotlin models, and frontend payloads all describe the same thing without mental translation.
