...
The `T−5 Max Delivery %` value now needs a hover detail showing each of the five preceding trading sessions with its date and delivery percentage. The response will return those existing source values with every trade, including a blank percentage for a missing delivery record; the tooltip is display-only and leaves all backtest calculations unchanged.
