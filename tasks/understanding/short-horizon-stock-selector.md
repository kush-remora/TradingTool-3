...
The default Shortlist gate now requires latest 5D net movement of at least +3%, at least 3 of the latest 5 sessions green, and basic historical reach of `3 / 20` or Recent tested 6D reach of `1 / 6`. Prior 5D remains visible context but is not a gate. The pace selector is optional and defaults to Any pace; it can further narrow results to Accelerating, Steady, Recovering, or Slowing. Recovering means both periods are negative but the latest decline improved by more than 1 percentage point. Strong finishes remain an optional minimum filter (`Any`, at least 3/5, at least 4/5, or 5/5). Volume activity remains visible and does not filter entry or exit. The existing rule that rejects only three falling closes plus a break below the previous five-session low remains active. The former 5/20, 2/6, and recent-high rules are not current-condition gates in Tab 2.
...
The current `Best aligned` tab inherits the Shortlist base rules: latest 5D movement of at least +3%, at least 3 / 5 latest sessions green, basic reach of `3 / 20` or Recent tested 6D reach of `1 / 6`, and the structural-weakness guard. It then adds at least `2 / 5` Strong finishes and allows only `Quiet` or `Watch` Volume activity; `Supply response` is excluded. Accelerating pace is not an additional Best aligned gate. Move quality and Volume activity remain visible context. The 52W-high column remains context and now shows the high price, distance from the high, high date, and trading sessions ago.
...
The obsolete `Best aligned · Quiet` and `Core` tabs have been removed. `Best aligned` is the next-level filter: all Shortlist base rules plus at least `2 / 5` Strong finishes. The separate Latest 2-day finish tab adds the latest-two-candles condition. These rules remain open for later review.

The former `Exit pressure` signal is now `Volume activity`. `Quiet` means no session in the latest five reached `1.5×` its preceding 10-session average; `Watch` means at least one did without a weak next-session response; `Supply response` means a qualifying volume event was followed by a red session closing in the lower 30% of its range. This is a cautionary price/volume state, not a confirmed distribution verdict. Best aligned excludes Supply response while keeping Quiet and Watch eligible.
