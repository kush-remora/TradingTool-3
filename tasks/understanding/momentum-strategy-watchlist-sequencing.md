...
The initial fixed rule was: event-day volume must be at least 2× the prior 5 trading-session average and RSI(14) must be at or below 35. If RSI is higher after five completed sessions, enter at the following session open and evaluate a 5% target for up to 15 holding sessions. There is no stop-loss in this first research version; unresolved trades are closed at the evaluation-window close so downside remains visible. Events are kept non-overlapping per stock, and no future candles are used to form an event or entry decision.

## Current decision: adaptive stock-specific RSI calibration

The fixed RSI <=35 gate will be replaced by a rolling stock-specific RSI ceiling. For each test event, use only completed historical volume-event setups from the preceding 252 trading sessions, bucket their event RSI values in 5-point bands, and choose the highest ceiling with enough samples and a better 5% target-hit rate than that stock's completed calibration baseline. If there is not enough calibration evidence, do not invent a fallback threshold; mark the event as insufficient RSI calibration. The six-month evaluation therefore requires roughly one year of warmup data before the test window.

The early-entry experiment keeps the same adaptive ceiling and price-context gates, then applies the simple past-data rule: RSI must improve from t-5 through t-1, excluding the event date t. If it passes, the simulated entry is the next session's open; the original future-five-session confirmation mode remains available as the benchmark. This is an earlier, lower-confirmation variant, so its target-hit rate must be compared directly with the benchmark rather than assumed equivalent.
