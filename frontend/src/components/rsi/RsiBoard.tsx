...
export function getExtensionColor(extensionPct: number, watchThresholdPct: number, skipThresholdPct: number): string | undefined {
...
export function renderEntryAction(row: RsiMomentumRankedStock) {
  const tag = (
...
export const holdingColumns: ColumnsType<RsiMomentumRankedStock> = [
  {
    title: "Rank",
    dataIndex: "rank",
    key: "rank",
...
export function buildGroupedCandidates(rows: RsiMomentumRankedStock[]) {
...
export function RsiMomentumBoardPanel({
  snapshot,
  watchThresholdPct,
  skipThresholdPct,
}: {
...
export function DiagnosticLine({ label, value }: { label: string; value: string }) {
  return (
...
export function DiagnosticTags({
  label,
  values,
  color,
}: {
...
export function LivePressureCell({ symbol }: { symbol: string }) {
