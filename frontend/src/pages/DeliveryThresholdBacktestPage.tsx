...
const rowColumns: ColumnsType<DeliveryThresholdBacktestRow> = [
  ...
  {
    title: "ROC 20D %",
    dataIndex: "roc20AtSignalPct",
    key: "roc20AtSignalPct",
    width: 120,
    sorter: (a, b) => (a.roc20AtSignalPct ?? 0) - (b.roc20AtSignalPct ?? 0),
    render: (value: number | null) => (value == null ? "-" : `${value.toFixed(2)}%`),
  },
  {
    title: "SMA 200",
    dataIndex: "sma200AtSignal",
    key: "sma200AtSignal",
    width: 110,
    sorter: (a, b) => (a.sma200AtSignal ?? 0) - (b.sma200AtSignal ?? 0),
    render: (value: number | null) => (value == null ? "-" : value.toFixed(2)),
  },
  {
    title: "Dist vs SMA200",
    dataIndex: "distFromSma200AtSignalPct",
    key: "distFromSma200AtSignalPct",
    width: 130,
    sorter: (a, b) => (a.distFromSma200AtSignalPct ?? 0) - (b.distFromSma200AtSignalPct ?? 0),
    render: (value: number | null) => (value == null ? "-" : `${value.toFixed(2)}%`),
  },
...
function downloadCsv(result: DeliveryThresholdBacktestResponse): void {
  ...
  const rows = result.rows.map((row) => [
    ...
    row.roc20AtSignalPct ?? "",
    row.sma200AtSignal ?? "",
    row.distFromSma200AtSignalPct ?? "",
    row.targetPrice,
