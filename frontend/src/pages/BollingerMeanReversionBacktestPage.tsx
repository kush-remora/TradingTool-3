...
import { useBollingerMeanReversionBacktest } from "../hooks/useBollingerMeanReversionBacktest";
import { getJson } from "../utils/api";
import type {
  BollingerMeanReversionBacktestConfig,
...
const DEFAULT_CONFIG: BollingerMeanReversionBacktestConfig = {
  capital: 200000,
  maxOpenPositions: 1,
  fromDate: "2026-01-01",
  toDate: "2026-05-16",
...
const tradeColumns: ColumnsType<BollingerMeanReversionBacktestTrade> = [
...
const debugColumns: ColumnsType<BollingerMeanReversionBacktestDebugRow> = [
  ...
  { title: "Signal", dataIndex: "signal", key: "signal", width: 140 },
];

export function BollingerMeanReversionBacktestPage() {
  const { data, error, loading, run } = useBollingerMeanReversionBacktest();
  ...
  const runBacktest = () => {
    setParseError(null);
    try {
      const config = JSON.parse(configText) as BollingerMeanReversionBacktestConfig;
      const request: BollingerMeanReversionBacktestRequest = {
        universe,
  ...
  return (
    <div style={{ padding: 24, background: "#f5f7fa", minHeight: "calc(100vh - 48px)" }}>
      <Space direction="vertical" size={16} style={{ width: "100%" }}>
        <Card size="small" title="Bollinger Mean Reversion Backtest Controls">
          <Space direction="vertical" style={{ width: "100%" }}>
            ...
            <Space>
              <Button type="primary" onClick={runBacktest} loading={loading}>Run Bollinger Mean Reversion Backtest</Button>
        ...
        {!loading && result && <BollingerMeanReversionBacktestResultView result={result} />}
...
function BollingerMeanReversionBacktestResultView({ result }: { result: BollingerMeanReversionBacktestResponse }) {
  const [debugTrade, setDebugTrade] = useState<BollingerMeanReversionBacktestTrade | null>(null);

  const columnsWithDebug: ColumnsType<BollingerMeanReversionBacktestTrade> = [
    ...tradeColumns,
    {
      title: "Debug",
      key: "debug",
