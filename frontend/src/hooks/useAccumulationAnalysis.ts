...
export function useAccumulationAnalysis() {
  ...
  const loadTimeline = useCallback(async (runId: number, symbol: string) => { const data = await getJson<AccumulationAnalysisTimeline>(`/api/strategy/accumulation-analysis/runs/${runId}/symbols/${symbol}`, { useCache: false }); setTimeline(data); return data; }, []);
  ...
  return { runs, summary, timeline, loading, error, loadRuns, loadSummary, loadTimeline, run };
}
