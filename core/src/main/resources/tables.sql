...
-- Generic per-instrument notes, shared by any stock screen.
...
-- Manual Chartink scanner evidence. Each row is one historical scanner hit.
CREATE TABLE IF NOT EXISTS public.chartink_scan_events (
    id               BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    source           TEXT        NOT NULL,
    universe_key     TEXT        NOT NULL,
    event_date       DATE        NOT NULL,
    symbol           TEXT        NOT NULL,
    marketcap_name   TEXT        NOT NULL,
    sector           TEXT        NOT NULL,
    source_file_name TEXT        NOT NULL,
    uploaded_at      TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    UNIQUE (source, universe_key, event_date, symbol)
);

CREATE INDEX IF NOT EXISTS idx_chartink_scan_events_date
    ON public.chartink_scan_events (event_date DESC);
CREATE INDEX IF NOT EXISTS idx_chartink_scan_events_symbol_date
    ON public.chartink_scan_events (symbol, event_date DESC);

CREATE TABLE IF NOT EXISTS public.accumulation_analysis_runs (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    universe_key TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS public.accumulation_case_snapshots (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    run_id BIGINT NOT NULL REFERENCES public.accumulation_analysis_runs(id) ON DELETE CASCADE,
    symbol TEXT NOT NULL,
    chain_start_date DATE NOT NULL,
    chain_end_date DATE NOT NULL,
    as_of_date DATE NOT NULL,
    chain_length_sessions INTEGER NOT NULL,
    ...
    sessions_to_breakout INTEGER,
    details JSONB NOT NULL DEFAULT '{}'::jsonb,
    UNIQUE (run_id, symbol, chain_start_date, chain_end_date, as_of_date)
);
...
CREATE TABLE IF NOT EXISTS public.phase_c_watchlist (
    ...
    indian_promoter_pct NUMERIC(10, 2),
    foreign_promoter_pct NUMERIC(10, 2),
    quarterly_gross_sales NUMERIC(18, 2),
